...
master.data <- readRDS("clean_data/clean_data.rds")
...
master.data$antimicrobial[master.data$antimicrobial=="multi-drug"] <- "MDR"
    ...
    for(reg in 1:length(region.list)){
     ...
     #        ylim = c(0, 10),
      ...
      #       alim = c(0,250),
             ...
             ilab = cbind(mydata$source_id, mydata$iso3, mydata$site_name, mydata$mid_year, mydata$QA, mydata$no_examined, mydata$result),
             ilab.pos = c(4, 4, 4, 4, 4, 4, 2),
             ilab.xpos = c(-60, -50, -40, -30, -20, -10, 110),
             cex = .5,
             refline = NA,
#             rows = rowpos,
             slab = NA)
      
      # text(c(-80, -55, -45, -20, -10), 8.2, pos = 4, cex = 0.5, font = 2, c("Study", "Country", "Site", "Year", "QA", "N"))
  ...
  for(sp in species.list){
    ...
    for(saplot in 1:3){
      if(saplot == 1){
        ...
        fileout <- paste0('results/meta_analysis/forest_plots/',
                          a,"_",
                          sp, "/",
      ...
      forest(x = mydata$percentage_resistant,
             ...
             digits = c(0),
             xlab = ("Percentage resistant (%)"),
             ilab = cbind(mydata$author_year, mydata$iso3, mydata$site_name, mydata$mid_year, mydata$no_examined, mydata$result),
...
fileout <- paste0('results/meta_analysis/forest_plots/FQR_Paratyphi/South_Asia_1995-2009.jpeg')
